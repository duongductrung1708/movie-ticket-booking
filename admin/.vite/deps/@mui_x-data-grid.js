...
function getCheckboxPropsSelector(groupId, autoSelectParents) {
  return createSelector2(gridRowTreeSelector, gridSortedRowIdsSelector, gridFilteredRowsLookupSelector, selectedIdsLookupSelector, (rowTree, sortedRowIds, filteredRowsLookup, rowSelectionLookup) => {
    ...
    return {
      isIndeterminate: selectedDescendantsCount > 0 && selectedDescendantsCount < selectableDescendantsCount || selectedDescendantsCount === selectableDescendantsCount && rowSelectionLookup[groupId] === void 0,
...
function GridBottomContainer(props) {
  ...
  const offset = Math.max(viewportOuterSize.height - minimumSize.height - // Subtract scroll height twice to account for GridVirtualScrollerFiller and horizontal scrollbar
  2 * scrollHeight, 0);
