...
const getShowtimesByRoomId = async (req, res) => {
    try {
        ...
        const responseData = showtimes
            ...
            .sort((a, b) => convertTo24HourFormat(a.start_time) - convertTo24HourFormat(b.start_time)) // Sort by start_time
            .map(st => {
                return {
                    _id: st._id,
                    movie: st.movie_id,
...
const createShowtime = async (req, res) => {
    try {
        ...
        res.status(201).json({ showtime: formattedShowtime, message: "Create Showtime Successfully" });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
};
...
const updateShowtime = async (req, res) => {
    try {
        ...
        res.status(200).json({
            message: "Update Showtime Successfully", showtime: formattedShowtime
        });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
