    ...
    try {
      ...
      if (formattedStartTime >= formattedEndTime) {
        return res
          ...
          .send({
            message:
              "Invalid time range: Start time must be less than end time.",
          });
      }
      ...
      for (let showtime of showtimes) {
        let isOverlapping =
          (formattedStartTime <= showtime.end_time &&
            formattedStartTime >= showtime.start_time) || // formattedStartTime falls within existing showtime range
          (formattedEndTime >= showtime.start_time &&
            formattedEndTime <= showtime.end_time) || // formattedEndTime falls within existing showtime range
          (formattedStartTime <= showtime.start_time &&
            formattedEndTime >= showtime.end_time); // new range contains the entire showtime range

        if (isOverlapping) {
          return res
            .status(400)
            .json({ message: "Showtime overlaps with an existing showtime." });
        }
      }
